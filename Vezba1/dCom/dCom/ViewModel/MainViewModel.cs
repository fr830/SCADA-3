		...
		private BasePointItem CreatePoint(ConfigItem c, int i, FunctionExecutor commandExecutor)
		{
			switch (c.RegistryType)
			{
				case PointType.DI_REG:
					return new DigitalInput(c.RegistryType, (ushort)(c.StartAddress + i), c.DefaultValue, commandExecutor, string.Format("{0} [{1}]", c.Description, i), this);
				case PointType.DO_REG:
					return new DigitalOutput(c.RegistryType, (ushort)(c.StartAddress + i), c.DefaultValue, commandExecutor, string.Format("{0} [{1}]", c.Description, i), this);
				case PointType.IN_REG:
					return new AnalaogInput(c.RegistryType, (ushort)(c.StartAddress + i), c.DefaultValue, commandExecutor, string.Format("{0} [{1}]", c.Description, i), this);
				case PointType.HR_INT:
					return new AnalogOutput(c.RegistryType, (ushort)(c.StartAddress + i), c.DefaultValue, commandExecutor, string.Format("{0} [{1}]", c.Description, i), this);
				default:
					return null;
			}
		}
