...
namespace ProcessingModule
{
	public class AutomationManager : IAutomationManager
    {
		...
		private void AutomationWorker_DoWork()
		{
            ...
            IConfigItem item = configuration.GetConfigurationItems().Find(x => x.StartAddress.Equals(1000));

			while (!shutDown)
			{
                ushort value = points[2].RawValue;

                if (points[0].RawValue.Equals(1))
                    value -= EGUConverter.ConvertToRaw(item.A, item.B, 10);

                if (points[1].RawValue.Equals(1))
                    value -= EGUConverter.ConvertToRaw(item.A, item.B, 15);

                if (!points[2].RawValue.Equals(value))
